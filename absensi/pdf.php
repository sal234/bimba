...
$arr=getData($conn,$sql);
		foreach($arr as $d) {	
  $cell[$i][0]=$d["kode_absensi"];
  $cell[$i][1]=$d["tgl"];
  $cell[$i][2]=$d["jam"];
  $cell[$i][3]=$d["kode_siswa"];
  $cell[$i][4]=$d["status"];
...
$pdf->Cell(2,1,'kode_absensi','LR',0,'C',1);
$pdf->Cell(7,1,'tgl','LR',0,'C',1);
$pdf->Cell(5,1,'jam','LR',0,'C',1);
$pdf->Cell(3,1,'kode_siswa','LR',0,'C',1);
$pdf->Cell(9,1,'status','LR',0,'C',1);
...
for ($j=0;$j<$i;$j++){
  ...
  $pdf->Cell(2,1,$cell[$j][0],'B',0,'L'); // kode_absensi
  $pdf->Cell(7,1,$cell[$j][1],'B',0,'L'); // tgl
  $pdf->Cell(5,1,$cell[$j][2],'B',0,'L'); // jam
  $pdf->Cell(3,1,$cell[$j][3],'B',0,'L'); // kode_siswa
  $pdf->Cell(9,1,$cell[$j][4],'B',0,'L'); // status

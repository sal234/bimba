...
<form action="" method="post" enctype="multipart/form-data">
	<table width="503">
		...
		<tr>
			...
			<td valign="top">:
			<td><input name="periode_ajaran" type="text" id="periode_ajaran" value="<?php echo $periode_ajaran; ?>" size="15" /></td>
		</tr>
		...
		<tr>
			...
			<td colspan="2"> <input name="Simpan" type="submit" id="Simpan" value="Simpan" />
				...
				<a href="?mnu=pendaftaransiswa"><input name="Batal" type="button" id="Batal" value="Batal" /></a>
			</td>
		</tr>
	</table>
</form>
...
<table width="100%" border="0">
	...
	<?php
	$sql = "select * from `$tbpendaftaran` where kode_siswa='" . $_SESSION["cid"] . "' order by `kode_pendaftaran` desc";
	$jum = getJum($conn, $sql);
	if ($jum > 0) {
		//--------------------------------------------------------------------------------------------
		$batas   = 10;
...
if ($jmldata > 0) {
	...
	if ($page > 1) {
		$prev = $page - 1;
		echo "<span class=prevnext><a href='$_SERVER[PHP_SELF]?page=$prev&mnu=pendaftaransiswa'>« Prev</a></span> ";
	} else {
	...
	for ($i = 1; $i <= $jmlhal; $i++)
		if ($i != $page) {
			echo "<a href='$_SERVER[PHP_SELF]?page=$i&mnu=pendaftaransiswa'>$i</a> ";
		} else {
	...
	if ($page < $jmlhal) {
		$next = $page + 1;
		echo "<span class=prevnext><a href='$_SERVER[PHP_SELF]?page=$next&mnu=pendaftaransiswa'>Next »</a></span>";
	} else {
...
if (isset($_POST["Simpan"])) {
	...
	$kode_siswa = strip_tags($_SESSION["cid"]);
	$tgl = date("Y-m-d");
...
'$kode_jadwal'
		...
		if ($simpan) {
			echo "<script>alert('Data $kode_pendaftaran berhasil disimpan !');document.location.href='?mnu=pendaftaransiswa';</script>";
		} else {
			echo "<script>alert('Data $kode_pendaftaran gagal disimpan...');document.location.href='?mnu=pendaftaransiswa';</script>";
		}
	} else {
...
where `kode_pendaftaran`='$kode_pendaftaran0'";
		...
		if ($ubah) {
			echo "<script>alert('Data $kode_pendaftaran berhasil diubah !');document.location.href='?mnu=pendaftaransiswa';</script>";
		} else {
			echo "<script>alert('Data $kode_pendaftaran gagal diubah...');document.location.href='?mnu=pendaftaransiswa';</script>";
		}
	} //else simpan
}
?>
...
if ($_GET["pro"] == "hapus") {
	...
	if ($hapus) {
		echo "<script>alert('Data pendaftaran $kode_pendaftaran berhasil dihapus !');document.location.href='?mnu=pendaftaransiswa';</script>";
	} else {
		echo "<script>alert('Data pendaftaran $kode_pendaftaran gagal dihapus...');document.location.href='?mnu=pendaftaransiswa';</script>";
	}
}
?>
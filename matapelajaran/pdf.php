...
class PDF extends FPDF{
  function Header(){
    ...
    $this->Cell(20,0,'Data matapelajaran',0,0,'L');
    $this->Ln();
    $this->Cell(5,1,'Laporan data matapelajaran',0,0,'L');
    $this->Ln();
...
$arr=getData($conn,$sql);
		foreach($arr as $d) {	
  $cell[$i][0]=$d["kode_matapelajaran"];
  $cell[$i][1]=$d["nama_matapelajaran"];
...
$pdf->Cell(2,1,'kode_matapelajaran','LR',0,'C',1);
$pdf->Cell(7,1,'nama_matapelajaran','LR',0,'C',1);
...
for ($j=0;$j<$i;$j++){
  ...
  $pdf->Cell(2,1,$cell[$j][0],'B',0,'L'); // kode_matapelajaran
  $pdf->Cell(7,1,$cell[$j][1],'B',0,'L'); // nama_matapelajaran
